...
3. Create a map using an program indicator from the COVID-19 Case-Based Surveillance program
   1. Create the chart using
      - Layer Type : Thematic 
      - Data:
...
### Create a map using an program indicator from the COVID-19 Case-Based Surveillance program
...
We will demonstrate this principles using two program indicators that use "Average" as the aggregation type. 
...
When we review this chart, we no longer see values that are representative of one individual person, but using the same indicator we are able to take an average for all individuals within the organisation unit and period that we have selected.
